...
%union{  // Later to be diccussed
	...
	Short_doubl		SHORT_DOUBLE              //Later disccus
}

%tokens FLOAT BOOL VOID CHAR ARRAY BYTE STRING DOUBLE FILE DIRECTORY LONG_INT SHORT_INT LONG_FLOAT SHORT_FLOAT  LONG_DOUBLE SHORT_DOUBLE BLN_FALSE BLN_TRUE AND_OPT OR_OPT NOT_OPT IF ELSE SWITCH CASE DEFAULT ELIF IFNOT WHILE DO BREAK CONTINUE FOR FUNCTION RETURN  BLTIN_PRINT BLTIN_PRINT BLTIN_LIST_CONTENTS BLTIN_GET_SIZE BLTIN_CREATE BLTIN_COPY BLTIN_COMPARE BLTIN_CONNECT_TO BLTIN_DELETE BLTIN_RENAME BLTIN_MOVE BLTIN_SORT BLTIN_FILTRE_FILES BLTIN_BACKUP BLTIN_SYNCHRONIZE BLTIN_SEARCH BLTIN_CD BLTIN_DOWNLOAD BLTIN_UPLOAD BLTIN_OPEN BLTIN_PROPERTIES BLTIN_MOVE_BACK BLTIN_MOVE_FORWARD INT FLOAT LONG_INT SHORT_INT AND_OPT NOT_OPT DEFAULT IFNOT DO BLTIN_PRINT LESSEQ_OPT GREATEREQ_OPT NEQ_OPT EQ_OPT LESS_OPT GREATER_OPT ASSIGNMENT_OPT MULTIPLY_ASSIGNMENT_OPT DIVIDE_ASSIGNMENT_OPT MODE_ASSIGNMENT_OPT ADD_ASSIGNMENT_OPT SUB_ASSIGNMENT_OPT POW_ASSIGNMENT_OPT INCREMENT_OPT DECREMENT_OPT NOT_OPT MULTIPLY_OPT DIVIDE_OPT MODE_OPT ADD_OPT SUB_OPT POW_OPT INT_LTRL FLT_LTRL STR_LTRL CHR_LTRL IDNTF SEMICOLON LEFT_BRACKET RIGHT_BRACKET COMMA COLON LEFT_PARANTHESIS RIGHT_PARANTHESIS LEFT_SQ_BRACKET RIGHT_SQ_BRACKET NEW_LINE WHITE_SPACE UNKNOWN_CHAR
